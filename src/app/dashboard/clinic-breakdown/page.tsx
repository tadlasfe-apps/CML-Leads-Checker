...
import type { ClinicBreakdownRow } from "@/types";
...
export default function ClinicBreakdownPage() {
  ...
  const [data, setData] = useState<ClinicBreakdownRow[]>([]);
  ...
  const filtered = data
    .filter((r) => !search || (r.clinicLocation ?? "").toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <TopBar title="Clinic Breakdown" description="Lead count reconciliation per clinic location" />
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="relative">
              ...
              <Input className="pl-9 w-48" placeholder="Search clinic…" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
          </div>
          <ExportButton endpoint="/api/export" filename="clinic-breakdown.csv"
        ...
        {loading ? (
          ...
          <>
            <Card>
              <CardHeader><CardTitle className="text-base">Leads by Source per Clinic (top 15)</CardTitle></CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={filtered.slice(0, 15)} margin={{ bottom: 60 }}>
            ...
            <Card>
              <CardHeader><CardTitle className="text-base">{filtered.length} Clinics</CardTitle></CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow className="text-xs">
                      <TableHead><SortHead field="clinicLocation" label="Clinic Location" /></TableHead>
                  ...
                  <TableBody>
                    {filtered.map((row) => (
                      <TableRow key={row.clinicLocation} className="text-sm">
